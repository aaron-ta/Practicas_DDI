...
{
    public class WatsonMQTT : MonoBehaviour
    {
            ...
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        if(ResultsField.text.Equals("lights on ")){
                            Log.Debug("ESTE ES UN IFFFFFFFFFFFFFF", "ENTRANDO AL IFFFF");
                            client.Publish("casa/sala/luz", System.Text.Encoding.UTF8.GetBytes("lights on"), MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE, true);
                        } 
                        else
                        if(ResultsField.text == "lights off ")
                            publish(lightTopic, "lights off");
                        else
                        if(ResultsField.text.Equals("check the temperature ")){
                            client.Publish("casa/sala/temperatura", System.Text.Encoding.UTF8.GetBytes("check temperature"), MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE, true);
                        }
